...
```
ansible-playbook -i inventory.ini ./proxmox-kubernetes-k3s/playbooks/helm/deploy_nfs_provisioner.yml  --extra-vars "masters=k3s_prod_masters minions=k3s_prod_minions proxmox=proxmox-01" 
```

Deploy Longhorn

```
ansible-playbook -i inventory.ini ./proxmox-kubernetes-k3s/playbooks/helm/deploy_longhorn.yml  --extra-vars "masters=k3s_prod_masters minions=k3s_prod_minions proxmox=proxmox-01" 
```

Deploy Devnotnull.com development resources

```
helm upgrade --install redis bitnami/redis --version 17.10.1 \
  --set master.service.type=LoadBalancer \
  --set replica.service.type=LoadBalancer \
  --set master.service.loadBalancerIP=10.150.10.141 \
  --set replica.service.loadBalancerIP=10.150.10.142 \
